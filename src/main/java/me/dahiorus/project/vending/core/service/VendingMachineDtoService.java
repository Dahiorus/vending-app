...
{
  /**
   * Reset all a vending machine statuses to OK. If the power status is OFF, the machine is restarted.
   *
   * @param id The vending machine ID
   * @return The reset machine
   * @throws EntityNotFound if no vending machine match the ID
   */
  VendingMachineDTO resetStatuses(UUID id) throws EntityNotFound;
}
