...
class VendingMachineDtoValidatorTest
{
  ...
  void dtoIsValid()
  {
    dto = buildDto("123456789", "1 Fake Street", ItemType.FOOD, PowerStatus.ON,
      WorkingStatus.OK);
  ...
  {
    dto = buildDto(serialNumber, "1 Fake Street", ItemType.FOOD, PowerStatus.ON,
      WorkingStatus.OK);

    ValidationResults results = validator.validate(dto);

    assertHasExactlyFieldErrors(results, "serialNumber",
  ...
  {
    dto = buildDto(RandomStringUtils.randomAlphabetic(256), "1 Fake Street",
      ItemType.FOOD, PowerStatus.ON,
      WorkingStatus.OK);
  ...
  {
    dto = buildDto("123456789", address, ItemType.FOOD, PowerStatus.ON,
      WorkingStatus.OK);

    ValidationResults results = validator.validate(dto);

    assertHasExactlyFieldErrors(results, "address.streetAddress",
      "validation.constraints.empty_value");
  }
  ...
  {
    dto = buildDto("1234", RandomStringUtils.randomAlphanumeric(256),
      ItemType.FOOD, PowerStatus.ON,
      WorkingStatus.OK);
  ...
  void placeHasMaxLength()
  {
    ...
    assertHasExactlyFieldErrors(results, "address.place",
      "validation.constraints.max_length");
  }
  ...
  {
    dto = buildDto("123456789", "123 rue Sésame", null, PowerStatus.ON,
      WorkingStatus.OK);
  ...
  {
    dto = buildDto("123456789", "123 rue Sésame", ItemType.COLD_BAVERAGE, null,
      null);

    ValidationResults results = validator.validate(dto);

    assertHasExactlyFieldErrors(results, "powerStatus",
      "validation.constraints.empty_value");
  }
  ...
  {
    dto = buildDto("123456789", "123 rue Sésame", ItemType.COLD_BAVERAGE,
      PowerStatus.OFF, WorkingStatus.ALERT);
  ...
  VendingMachineDto buildDto(final String serialNumber, final String address,
    final ItemType type,
